...
Adapted from https://github.com/tensorflow/tensorflow/blob/679bb02274de245c3ce00d2229081010c656dc3c/tensorflow/examples/image_retraining/retrain.py

Expected folder Structure:

ImageData/<name>/<"arriving" or "leaving">/image1.png
ImageData/<name>/<"arriving" or "leaving">/image2.png
...
def create_image_lists(image_dir, testing_percentage,
    ...
    if not gfile.Exists(image_dir):
        ...
        return None
    result = {}
    sub_dirs = get_immediate_subdirectories(image_dir)
    ...
    for sub_dir in sub_dirs:
        ...
        if dir_name == image_dir:
            continue
        for a_type in ['arriving', 'leaving']:
            file_list = []
            real_dir_name = dir_name
            dir_name = os.path.join(dir_name, a_type)
            ...
            if os.path.isdir(os.path.join(image_dir, dir_name)):
                ...
                if len(file_list) < 2:
                    print('WARNING: needs more images.')
                    continue
                ...
                label_name = re.sub(r'[^a-z0-9]+', ' ',
                                    real_dir_name.lower()).replace(" ", "_") + '_' + a_type
                training_images = []
                testing_images = []
                validation_images = []
    ...
    with tf.Graph().as_default() as graph:
        ...
        with gfile.FastGFile(model_filename, 'rb') as f:
            ...
            (bottleneck_tensor, jpeg_data_tensor,
             resized_input_tensor) = tf.import_graph_def(graph_def,
                    name='', return_elements=[BOTTLENECK_TENSOR_NAME,
...
def get_or_create_bottleneck(
  ...
  """
    ...
    bottleneck_path = get_bottleneck_path(image_lists, label_name,
            index, bottleneck_dir, category)
...
def get_random_distorted_bottlenecks(
  ...
  """
    ...
    # might be optimized in other implementations.
        ...
        bottleneck = run_bottleneck_on_image(sess,
                distorted_image_data, resized_input_tensor,
                bottleneck_tensor)
        ground_truth = np.zeros(class_count, dtype=np.float32)
        ground_truth[label_index] = 1.0
        bottlenecks.append(bottleneck)
...
def main(_):
  ...
  # See if the command-line flags mean we're applying any distortions.
    ...
    # some new images we haven't used before.
        ...
        print('Final test accuracy = %.1f%% (N=%d)' % (test_accuracy
